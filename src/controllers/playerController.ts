...
const getAllPlayers = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
...
const getPlayerById = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.user) {
             res.status(401).json({ error: "Unauthorized" });
             return;
...
const getActivePlayers = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.user) {
            res.status(401).json({ error: "Unauthorized" });
            return;
...
const getPossibleTeammates = async (req: AuthRequest, res: Response): Promise<void> => {
...
const getRankings = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        const playersStats = await Promise.all(players.map(async (player) => {
...
const getPlayerStats = async (req: AuthRequest, res: Response): Promise<void> => {
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
export { getAllPlayers, getPlayerById, createPlayer, getActivePlayers, updateStatus, getPossibleTeammates, getPlayerStats, getRankings };
